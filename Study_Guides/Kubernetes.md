...
## Kubernetes: Object Types & Manifests

### Manifests
YAML files that contain the configurations for Kubernetes objects are called manifest files. There are some key sections to be aware of in these files:
- kind
    - this tells Kubernetes what type of object is being configured
        - pod, service, ingress, etc
- metadata 
    - this section contains high level data about the object, and can be used for identifying and connecting objects in order for Kubernetes to achieve the desired state
        - name
        - annotations
            - these can modify the object further, often used with helm charts
        - labels
            - key value pairs that are often used as selectors
- spec
    - this section is where technical information about the object goes
        - details on the container to run in a pod, what selector a service uses to find its target pods, etc
...
## Helm
Helm is a tool that allows you to install and manage "packages" in Kubernetes. These "packages" helm works with are referenced as charts, and they come from various repositories. You tell Helm what repositories to get charts from, and then you can create a "release" of the chart. A release is whatever objects are created by the chart in the cluster, and it gives you a way to update those resources as a group. So, the workflow looks like this:
...
2. deploy the chart
```cli
helm upgrade --install {release name} {chart to deploy}
```
Your cluster will then have access to the objects generated by the chart, and you can begin to utilize them